...
class Dash_Controller extends Template_Controller {
	...
	public function autocomplete() {
		...
		//$db =& MDB2::connect($dsn);
		//if (PEAR::isError($db)) {
			//return "Couldn't access database: " . $db->getMessage();
		//}

		//$type = (isset($type)) ? "AND type='".$type."'" : '';

		//$query = "SELECT DISTINCT {$val} FROM sermons WHERE {$val} LIKE '%{$q}%' {$type} ORDER BY {$val} LIMIT {$limit}";
